...
describe('filter()', () => {
  ...
  it('filters an object to only numeric values', () => {
    ...
    const description = _.filter(characterAttributes, (value) => isNaN(value));
    expect(description).toEqual(['Thokul Mongothsbeard', 'Lord Thokul Mongothsbeard the Arcane', 'human', 'druid']);
  });
});